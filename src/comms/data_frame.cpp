#include "ArduIrrigationBase/comms/data_frame.h"
...
void DataFrame::copyContentsToArray(uint8_t* dataArray, uint32_t endIndex, uint32_t startIndex, uint32_t offset) const {
    ...
    for (uint32_t i = startIndex; i < endIndex; i++) dataArray[i+offset-startIndex] = data[i];
...
uint8_t* DataFrame::getDataPointer() {
    return data;
}
